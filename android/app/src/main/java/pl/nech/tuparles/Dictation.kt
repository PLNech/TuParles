...
object Dictation {
     ...
     */
    suspend fun decode(samples: ShortArray, lang: String, postprocessOn: Boolean): Take {
        ...
        val raw = withTimeout(DECODE_TIMEOUT_MS) {
            withContext(Dispatchers.Default) {
                ctx.transcribeData(samples.toFloats(), printTimestamp = false, language = lang).trim()
            }
        }
