...
class RoomNotesRepository @Inject constructor(
    ...
    // Deleting a note prunes its committed segments too (no FK; explicit and testable).
    override suspend fun delete(note: Note) {
        dao.deleteSegmentsFor(note.id)
        dao.delete(note)
    }

    override suspend fun get(id: Long): Note? = dao.get(id)
    override suspend fun pendingTranscripts(): List<Note> = dao.pendingTranscripts()
    override suspend fun recordingNotes(): List<Note> = dao.recordingNotes()
    override suspend fun addSegment(segment: NoteSegment): Long = dao.insertSegment(segment)
    override suspend fun segmentsFor(noteId: Long): List<NoteSegment> = dao.segmentsFor(noteId)
